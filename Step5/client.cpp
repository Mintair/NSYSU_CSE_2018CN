...
int rcv_msg()
{
	...
	{
		//Record first packet's ack_num
		if(rcv_pkt.seq_num == 1)
		{
			init_rcv_ack_num = rcv_pkt.ack_num;
			prev_num = rcv_pkt.ack_num - 1;
		}
		//Reset sending buffer
		memset(snd_pkt.data , 0 , MSS);
		//Receive in right order
		if((rcv_pkt.ack_num - prev_num == 1) && rcv_pkt.offset_reserved_flag == 0)
		{
			prev_num ++;
			//Write in the file
