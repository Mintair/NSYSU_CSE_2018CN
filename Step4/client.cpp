...
int client_fd,server_port,pkt_count,ack_num = 1;
string server_ip;
struct sockaddr_in client_addr,server_addr;
struct timeval tv;//timeout setting
fd_set rfd;//the fd set for select
...
int rcv_msg()
{
	...
	//Packet initialize
	pkt_init();
	//Reset the fd set rfd
	FD_ZERO(&rfd);
	//Delay time limit,500ms
	tv.tv_usec = 500000;
	//Put client_fd to rfd
	...
	while(recvfrom(client_fd , &rcv_pkt , pkt_size , 0 , (struct sockaddr*)&server_addr , &socklen)!=-1)
	{
		...
		memset(snd_pkt.data , 0 , MSS);
		//Calculate the ack num
		ack_num += rcv_pkt.window_size;
		//Set ack_num and seq_num
		snd_pkt.ack_num = ack_num;
		snd_pkt.seq_num = rcv_pkt.ack_num;
		//Calculate total received packet
		pkt_count ++;
		if(rcv_pkt.offset_reserved_flag == 0)
		{
			...
			memset(rcv_pkt.data, 0 , MSS);
			//2 pkt -> 1 ack and time < 500ms (select return 0 if timeout) timeout error ???
			if(pkt_count%2 && select(client_fd+1 , &rfd , 0 , 0 , &tv))
				continue;
		}
